...
from app.projects.betfake.models import (
    BetfakeBet, BetfakeGame, BetfakeAccount, 
    BetfakeTransaction, BetStatus, TransactionType, MarketType, GameStatus
)
...
class BetGraderService:
    ...
    def settle_pending_bets(self, game_id=None):
        ...
        if game_id:
            ...
            query = query.join(BetfakeOutcome).join(BetfakeMarket).filter(BetfakeMarket.game_id == game_id)
