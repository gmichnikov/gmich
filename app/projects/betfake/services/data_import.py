...
from app.projects.betfake.models import (
    BetfakeGame, BetfakeMarket, BetfakeOutcome, 
    GameStatus, MarketType
)
...
class DataImportService:
    ...
    def import_odds_for_sport(self, sport_key, markets_str="h2h,spreads,totals"):
            ...
            for bookmaker in event['bookmakers']:
                ...
                for market in bookmaker['markets']:
                    ...
                    if not active_market:
                        ...
                        # Compare outcomes
                        existing_outcomes = {o.outcome_name: (o.odds, o.point_value) for o in active_market.outcomes}
                        new_outcomes = {o['name']: (o['price'], o.get('point')) for o in market['outcomes']}
        ...
        db.session.commit()
        logger.info(f"Imported {market_count} markets and {outcome_count} outcomes for {sport_key}")
        return market_count
            ...
            for bookmaker in event['bookmakers']:
                ...
                for market in bookmaker['markets']:
                    ...
                    if not active_market:
                        ...
                        existing_outcomes = {o.outcome_name: o.odds for o in active_market.outcomes}
                        new_outcomes = {o['name']: o['price'] for o in market['outcomes']}
                        
                        if len(existing_outcomes) != len(new_outcomes) or existing_outcomes != new_outcomes:
                            should_create_new = True
                            active_market.is_active = False
                            active_market.marked_inactive_at = datetime.utcnow()
