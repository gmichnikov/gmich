...
(function () {
  ...
  const commandPillWrapper = document.getElementById('docs-demo-command-pill-wrapper');
  ...
  const SLASH_COMMANDS = {
    '/improve': {
      ...
      prompt: 'Rewrite the following text to be clearer, more concise, and more professional. Return only the improved text, no explanation:\n\n{text}',
      requiresSelection: false,
      type: 'text',
    },
    '/expand': {
      label: 'Add detail, elaborate',
      prompt: 'Elaborate on the following text with more detail and supporting points. Return only the expanded text, no explanation:\n\n{text}',
      requiresSelection: false,
      type: 'text',
    },
    '/summarize': {
      label: 'Condense to summary',
      prompt: 'Condense the following text into a shorter summary. Return only the summary, no explanation:\n\n{text}',
      requiresSelection: false,
      type: 'text',
    },
    '/brainstorm': {
      label: 'Bullet-point ideas',
      prompt: 'Generate exactly 3 to 5 bullet-point ideas about the following topic. Return only the bullet list (use - for each item), no intro text, no explanation, no extra items:\n\n{text}',
      requiresSelection: true,
      type: 'text',
    },
    '/review': { label: 'Passage-level feedback', type: 'comments', requiresSelection: false },
    '/comment': { label: 'e.g. /comment look for passive voice', type: 'comments', requiresSelection: false, requiresExtraText: true },
  };

  /* ----- Document helpers ----- */
  function getDocumentText() {
    return (pageContent && pageContent.innerText) ? pageContent.innerText.trim() : '';
  }
  ...
  function clearSelectionMarker() {
    if (selectionMarkerSpan && selectionMarkerSpan.parentNode) {
      ...
      while (selectionMarkerSpan.firstChild) {
        parent.insertBefore(selectionMarkerSpan.firstChild, selectionMarkerSpan);
      }
  ...
  function captureDocSelection() {
    const sel = window.getSelection();
    if (sel && sel.rangeCount > 0 && pageContent && pageContent.contains(sel.anchorNode)) {
  ...
  function applyTextResult(text, isBrainstorm, hadSelection) {
    ...
    if (isBrainstorm && hasBullets) {
      ...
      if (selectionMarkerSpan && selectionMarkerSpan.parentNode) {
        selectionMarkerSpan.parentNode.insertBefore(ul, selectionMarkerSpan.nextSibling);
      } else if (savedSelection && pageContent.contains(savedSelection.commonAncestorContainer)) {
    ...
    } else if (hadSelection && savedSelection && pageContent.contains(savedSelection.commonAncestorContainer)) {
  ...
  function replaceFullDocument(text) {
    ...
    if (lines.length <= 1) {
      ...
      pageContent.innerHTML = lines.map(function (l) {
  ...
  function handleSubmit() {
    const raw = (commandInput && commandInput.value) ? commandInput.value.trim() : '';
    if (!raw) return;
  ...
  function handleCommentsCommand(cmd, extra) {
    if (cmd === '/comment' && !extra) {
      window.docsDemo.showError('Please add instructions, e.g. /comment look for passive voice');
      return;
    }
  ...
  function callGenerateText(prompt, text, isBrainstorm, hadSelection) {
    ...
    fetch(typeof API_BASE !== 'undefined' ? API_BASE : '/docs-demo/api/generate', {
      method: 'POST',
      ...
      .catch(function (err) {
        window.docsDemo.showError(err.message || 'Something went wrong. Please try again.');
      })
      .finally(function () {
  ...
  function callGenerateComments(cmd, extra) {
    ...
    const reviewPrompt = 'You are a writing editor. Review this document and provide passage-level feedback. Return a JSON array of objects, each with "target" (exact quote from the document) and "comment" (your feedback). Quote the target text exactly as it appears. Keep each comment to 1-2 sentences. Example: [{"target":"exact phrase","comment":"Brief feedback here."}]';
    const commentPrompt = extra
      ? 'You are a writing editor. The user wants you to look for: "' + extra + '". Read the document and add comments where appropriate. Return a JSON array of objects with "target" (exact quote) and "comment". Quote targets exactly. Keep each comment to 1-2 sentences.'
      : reviewPrompt;
    ...
    fetch(typeof API_BASE !== 'undefined' ? API_BASE : '/docs-demo/api/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'comments', prompt: prompt, text: docText }),
    })
      .then(function (r) {
        if (!r.ok) throw new Error(r.statusText);
        return r.json();
  ...
  function renderComments(comments) {
    ...
    highlights.sort(function (a, b) { return a.idx - b.idx; });
  ...
  function applyHighlights(highlights) {
    ...
    highlights.sort(function (a, b) { return b.idx - a.idx; });
    highlights.forEach(function (h) {
      ...
      for (let i = 0; i < map.length; i++) {
        ...
        const middle = m.node.textContent.substring(overlapStart - m.start, overlapEnd - m.start);
  ...
  function bindCommentClicks(highlights) {
    ...
    function activateComment(id) {
      commentsGutter.querySelectorAll('.docs-demo-comment-card').forEach(function (c) { c.classList.remove('docs-demo-active'); });
      pageContent.querySelectorAll('.docs-demo-comment-highlight').forEach(function (h) { h.classList.remove('docs-demo-active'); });
      const card = commentsGutter.querySelector('.docs-demo-comment-card[data-comment-id="' + id + '"]');
      if (card) {
        card.classList.add('docs-demo-active');
        card.scrollIntoView({ block: 'nearest' });
      }
      pageContent.querySelectorAll('.docs-demo-comment-highlight[data-comment-id="' + id + '"]').forEach(function (h) {
        h.classList.add('docs-demo-active');
      });
    }

    pageContent.querySelectorAll('.docs-demo-comment-highlight').forEach(function (hl) {
      hl.addEventListener('click', function () {
        activateComment(hl.dataset.commentId);
      });
    });
    commentsGutter.querySelectorAll('.docs-demo-comment-card').forEach(function (card) {
      card.addEventListener('click', function () {
        const id = card.dataset.commentId;
        const hl = pageContent.querySelector('.docs-demo-comment-highlight[data-comment-id="' + id + '"]');
  ...
  function selectAutocompleteItem(cmd) {
    if (commandInput) {
      commandInput.value = cmd + (SLASH_COMMANDS[cmd] && SLASH_COMMANDS[cmd].requiresExtraText ? ' ' : '');
      commandInput.focus();
    }
  ...
  function updateAutocompleteSelection() {
    ...
    const items = autocompleteEl.querySelectorAll('.docs-demo-autocomplete-item');
    items.forEach(function (el, i) {
      el.classList.toggle('docs-demo-selected', i === autocompleteSelectedIndex);
    });
  ...
  function showAutocomplete(filter) {
    ...
    autocompleteFilteredCommands = !filter
      ? cmdList
      : cmdList.filter(function (c) { return c.indexOf(filter) === 0 || c.replace('/', '').indexOf(filter.replace('/', '')) === 0; });
    if (autocompleteFilteredCommands.length === 0) {
      autocompleteEl.classList.remove('docs-demo-visible');
      return;
    }
    autocompleteSelectedIndex = 0;
    autocompleteEl.innerHTML = autocompleteFilteredCommands.map(function (cmd) {
      const def = SLASH_COMMANDS[cmd];
      const label = def ? def.label : cmd;
      return '<div class="docs-demo-autocomplete-item" data-cmd="' + escapeHtml(cmd) + '">' + escapeHtml(cmd) + ' <span>' + escapeHtml(label) + '</span></div>';
    }).join('');
    autocompleteEl.classList.add('docs-demo-visible');
    autocompleteEl.querySelectorAll('.docs-demo-autocomplete-item').forEach(function (el) {
  ...
  if (commandInput) {
    ...
    commandInput.addEventListener('keydown', function (e) {
      const menuOpen = autocompleteEl && autocompleteEl.classList.contains('docs-demo-visible');

      if (menuOpen) {
        if (e.key === 'ArrowDown') {
          e.preventDefault();
          autocompleteSelectedIndex = (autocompleteSelectedIndex + 1) % autocompleteFilteredCommands.length;
          updateAutocompleteSelection();
          return;
        }
        if (e.key === 'ArrowUp') {
          ...
          if (autocompleteSelectedIndex < 0) autocompleteSelectedIndex = autocompleteFilteredCommands.length - 1;
          updateAutocompleteSelection();
          return;
        }
  ...
  if (helpBtn) {
    helpBtn.addEventListener('click', function () {
      ...
      popup.style.cssText = 'position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#fff;border:1px solid #dadce0;border-radius:12px;padding:16px 20px;box-shadow:0 4px 20px rgba(0,0,0,0.2);z-index:1000;max-width:480px;';
      const tableRows = Object.keys(SLASH_COMMANDS).map(function (cmd) {
        ...
        return '<tr><td class="docs-demo-help-cmd"><code>' + escapeHtml(code) + '</code></td><td class="docs-demo-help-desc">' + escapeHtml(label) + '</td><td class="docs-demo-help-sel">' + escapeHtml(selLabel) + '</td></tr>';
      }).join('');
      popup.innerHTML = '<div class="docs-demo-help-title">Commands</div>' +
        '<table class="docs-demo-help-table"><thead><tr><th>Command</th><th>Description</th><th>Operates on</th></tr></thead><tbody>' + tableRows + '</tbody></table>';
