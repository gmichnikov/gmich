...
class LLMQuestion(db.Model):
    ...
    summary_response_id = db.Column(db.Integer, db.ForeignKey('llm_response.id'), nullable=True)
    
    user = db.relationship('User', backref=db.backref('llm_questions', lazy=True))
    responses = db.relationship('LLMResponse', 
                              backref=db.backref('llm_question', lazy=True), 
                              foreign_keys='LLMResponse.question_id',
                              cascade='all, delete-orphan')
    summary_response = db.relationship('LLMResponse', 
...
class LLMResponse(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    question_id = db.Column(db.Integer, db.ForeignKey('llm_question.id'), nullable=False)
    llm_name = db.Column(db.String(50), nullable=False)  # e.g., 'GPT-5 Mini', 'Claude Haiku 4.5', 'Gemini 2.5 Flash'
    model_name = db.Column(db.String(100), nullable=False)  # e.g., 'gpt-5-mini', 'claude-haiku-4-5', 'gemini-2.5-flash'
    content = db.Column(db.Text, nullable=False)
    timestamp = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
