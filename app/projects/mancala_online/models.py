...
class MancalaOnlineRoom(db.Model):
    __tablename__ = "mancala_online_room"
    ...
    # Board is 14 ints:
    # 0..5: Player X pits (left to right from X's perspective)
    # 6: Player X store (right store for X)
    # 7..12: Player O pits (left to right from O's perspective)
    # 13: Player O store (right store for O)
    ...
    winner = db.Column(db.String(4), nullable=True)  # "X", "O", or "draw"
    ...
    def __repr__(self):
        return f"<MancalaOnlineRoom {self.code} status={self.status}>"
